...
def main(img_pattern: str,
    ...
    if not video:
        for name, pair in tqdm(zip(names, pairs), total=len(names), disable=True):
            f_img, f_mask = pair
    ...
    return start_time-init_time, elapsed_time
...
def get_files():
    list=[]
    for filepath,dirnames,filenames in os.walk(r'.\dataset1\ECE549_Final_Project_Dataset'):
...
if __name__ == '__main__':
    ...
    img_path=get_files()
    for i in img_path:
        init_time, infer_time = main(i)
        total_init_time += init_time
        total_inference_time += infer_time
    print("Init time average:", total_init_time/280.)
    print("Infer time average:", total_inference_time/280.)
